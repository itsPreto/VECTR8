...
class MBedFastAF:
    def __init__(self, model_name, similarity_metric='cosine', db_filename=None):
...
def preview_document():
    data = request.get_json()
    logger.info(f"Received data for preview_document: {data}")
    file_path = data.get('file_path')
    ...
    # Select a single document for preview
    if documents:
        document = documents[0]
    else:
        document = ""

    embeddings = library.get_embeddings([document])
    token_count = sum([len(library.tokenizer.tokenize(doc)) for doc in [document]])
...
def extract_texts_from_json(data, keys):
    logger.info(f"Extracting texts from json: {data} with keys: {keys}")
    documents = []
    for item in data:
        texts = []
        for key in keys:
...
def extract_texts_from_csv(df, keys):
    logger.info(f"Extracting texts from csv: {df} with keys: {keys}")
    documents = []
...
def create_vector_database():
    global current_progress, library, db_filename
    current_progress = 0
    data = request.get_json()
    file_path = data.get('file_path')
    selected_keys = data.get('selected_keys', [])
    ...
    # Create a new instance of MBedFastAF with the new db_filename
    library = MBedFastAF('nomic-ai/nomic-embed-text-v1.5-GGUF', 'euclidean', db_filename)
...
def chatbot_format():
    ...
    if response.status_code == 200:
        ...
        return jsonify({'error': 'Failed to communicate with the inferencing API'}), 500
